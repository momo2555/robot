...
class Driver:
  ...
  #def _sign_extend(self, val, orig):
    #return ((val) & (1 << ((orig) - 1))) ? (val | (((1 << (1 + (sizeof(val) << 3) - (orig))) - 1) << (orig))) : val)
   # return (val,(val | (((1 << (1 + (sizeof(val) << 3) - (orig))) - 1) << (orig)))) [((val) & (1 << ((orig) - 1)))]
  # Get Device ID.
  ...
  # This is used for applying manual calibration constants if required.
  # For auto-calibration, @see autoCalibrateZAccelOffset(0).
  # @see getZAccelOffset()
  # @see registers.OFFSET_2
